<template>
  <div class="main flex">
    <div class="tagBox tagli" v-if="tag || (classify && classify != 'all')">
      <Icon type="ios-close-circle" @click="deleteTag"/>
      {{ tag || classify }}
    </div>

    <TextLoading v-if="loading"> </TextLoading>
    <div class="article slide" v-else>
      <router-link :to="{ path: `/blog/${item.id}` }" class="list animate03" v-for="(item, index) in articles"
        :key="index">
    ...
    <Skills :pageModel="pageModel" @getArticles="getArticles" @ArticlesOrderByClassify="ArticlesOrderByClassify"
...
export default {
  ...
  methods: {
    ...
    ArticlesOrderByClassify() {
      ...
      this.$api
        .ArticleList(Object.assign(param, this.pageModel))
        .then((res) => {
          this.pageModel.sumCount = res.data.total
          this.articles = res.data.data
...
.list:hover {
  box-shadow: 1 1px 20px -6px rgba(0, 0, 0, 0.5);
