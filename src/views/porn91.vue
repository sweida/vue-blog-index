<template>
  <mu-container>
    ...
    <mu-flex justify-content="center" style="margin: 32px 0;">
      <mu-pagination raised :total="porn91.length" :page-size="pageSize" :page-count=5 :current.sync="current" @change="handlpage"></mu-pagination>
    </mu-flex>
  </mu-container>
</template>

<script>
import porn91 from '@/data/porn91/index.js'
export default {
  ...
  methods: {
    ...
    // 获取分页数据
    goPage() {
      this.listData = this.porn91.slice( (this.current-1)*this.pageSize, this.current*this.pageSize)
    ...
    // 点击分页
    handlpage() {
      this.$router.push({path: '/porn91', query: {page: this.current}})
      const loading = this.$loading();
      this.goPage()
      setTimeout(() => {
        loading.close()
