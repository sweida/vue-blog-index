<template>
  <mu-container>
    ...
    <mu-flex justify-content="center" style="margin: 32px 0;">
      <mu-pagination raised :total="riji.length" :page-size="pageSize" :page-count=5 :current.sync="current" @change="handlpage"></mu-pagination>
    </mu-flex>
  </mu-container>
</template>

<script>
import riji from '@/data/riji/index.js'
export default {
  data () {
    return {
      name: '嫖客日记',
  ...
  methods: {
    ...
    // 获取分页数据
    goPage() {
      this.listData = this.riji.slice( (this.current-1)*this.pageSize, this.current*this.pageSize)
    ...
    // 点击分页
    handlpage() {
      this.$router.push({path: '/riji', query: {page: this.current}})
      const loading = this.$loading();
      this.goPage()
      setTimeout(() => {
        loading.close()
