<template>
  <mu-container>
    ...
    <mu-flex justify-content="center" style="margin: 32px 0;">
      <mu-pagination raised :total="hanguo.length" :page-size="pageSize" :page-count=5 :current.sync="current" @change="handlpage"></mu-pagination>
    </mu-flex>
  </mu-container>
</template>

<script>
import hanguo from '@/data/hanguo/index.js'
export default {
  data () {
    return {
      name: '韩国主播',
  ...
  methods: {
    ...
    // 获取分页数据
    goPage() {
      this.listData = this.hanguo.slice( (this.current-1)*this.pageSize, this.current*this.pageSize)
    ...
    // 点击分页
    handlpage() {
      this.$router.push({path: '/hanguo', query: {page: this.current}})
      const loading = this.$loading();
      this.goPage()
      setTimeout(() => {
        loading.close()
